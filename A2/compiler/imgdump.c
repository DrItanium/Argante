...
static FILE *in;
static symbol *code_symbol, *data_symbol, *curr_symbol;

static int disassemble_code, disassemble_data, revert_symbols;

static void section_dump(void);
static void disassemble(void);
static void dump_code(struct bcode_op *arg, unsigned offs);
static void dump_data(struct data_pkt *arg, char *last_type, unsigned offs);

/* Print d in hex or decimal, whichever is better. */
static void represent_int(FILE *to, int d)
{
	int old_d=d;
...
static symbol *find_symbol(symbol *head, unsigned int addr)
{
	symbol *x;
	x=head;
	while (x)
...
static symbol *find_reloc_in(symbol *head, unsigned int place, unsigned int addr, areloc **ret)
{
	symbol *x;
	areloc *r;
	x=head;
...
static symbol *find_reloc(unsigned int place, unsigned addr, areloc **ret)
{
	symbol *s;
...
static void represent_reloc(FILE *to, symbol *s, areloc *r)
{
	switch(r->r.type)
	{
		case RELOC_ADDR:
...
static void dump_codearg(int type, anyval *a1, int offs)
{
	char isreg, ispoint;
	symbol *s;
	areloc *r;
...
static void dump_code(struct bcode_op *arg, unsigned offs)
{
	...
	{
		printf("\t%s", op[arg->bcode].name);
		args=op[arg->bcode].params;
	if (revert_symbols && (!strcasecmp(op[arg->bcode].name, "SYSCALL") ||
				!strcasecmp(op[arg->bcode].name, "SYSCALL2")))
	{
		for (i=0;i<SCNUM;i++)
		{
	...
	if (args < 1) { printf("\n"); return; }
	printf("\t");
	dump_codearg(arg->type, &arg->a1, offs * sizeof(struct bcode_op) + OFFSET_OP_A1);
arg2:
	if (args < 2) { printf("\n"); return; }
	printf(", ");
	dump_codearg(arg->type >> 4, &arg->a2, offs * sizeof(struct bcode_op) + OFFSET_OP_A2);
	printf("\n"); 
}
	...
	while(fread(&mess_desc, sizeof(mess_desc), 1, in))
	{
		...
		switch(mess_desc.type)
		{
			...
			case BFMT_CODE: {
				 ...
				 * sizeof(bopp)'s not bytes. Well... I hate myself :)
				 */
				printf("TYPE: %16.16s, SIZE: 0x%x\n", "BFMT_CODE", mess_desc.size);
				i=0;
				while (i < mess_desc.size)
				{
					frread(&bopp, sizeof(bopp), 1, in);
...
static void disassemble(void) {
	...
	for (j=0;j<data_segments;j++) {
		if (j==0 || ds_readonly[j] != ds_readonly[j-1]) {
			if (ds_readonly[j] == 0) printf(".DATA\n");
			else printf(".RODATA\n");
		}
		...
		for (i=0;i<data_segment_size[j];i++) {
			if (revert_symbols)
				s=find_symbol_data(i + A2_MAX_BLKSIZ * j);
			else
				s=NULL;
			...
			dump_data(&data_segment[j][i], &last_type, i + A2_MAX_BLKSIZ * j);
		}
		if (last_type == DTYPE_STRING) printf("\"\n");
	}
	if (hidden) printf("}\n");
