...
#define NEW_IP(x) if (x > curr_cpu->csize) throw_except(curr_cpu, ERR_OUTSIDE_CODE); else curr_cpu->ip=x
...
static void cmd_ifeq_uu(JIT_ARGS) { if (a1->val.u != a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifeq_us(JIT_ARGS) { if (a1->val.u != a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifeq_uf(JIT_ARGS) { if (a1->val.u != a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifeq_su(JIT_ARGS) { if (a1->val.s != a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifeq_ss(JIT_ARGS) { if (a1->val.s != a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifeq_sf(JIT_ARGS) { if (a1->val.s != a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifeq_fu(JIT_ARGS) { if (a1->val.f != a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifeq_fs(JIT_ARGS) { if (a1->val.f != a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifeq_ff(JIT_ARGS) { if (a1->val.f != a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
...
static void cmd_ifneq_uu(JIT_ARGS) { if (a1->val.u == a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifneq_us(JIT_ARGS) { if (a1->val.u == a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifneq_uf(JIT_ARGS) { if (a1->val.u == a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifneq_su(JIT_ARGS) { if (a1->val.s == a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifneq_ss(JIT_ARGS) { if (a1->val.s == a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifneq_sf(JIT_ARGS) { if (a1->val.s == a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifneq_fu(JIT_ARGS) { if (a1->val.f == a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifneq_fs(JIT_ARGS) { if (a1->val.f == a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifneq_ff(JIT_ARGS) { if (a1->val.f == a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
...
static void cmd_ifabo_uu(JIT_ARGS) { if (a1->val.u > a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifabo_us(JIT_ARGS) { if (a1->val.u > a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifabo_uf(JIT_ARGS) { if (a1->val.u > a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifabo_su(JIT_ARGS) { if (a1->val.s > a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifabo_ss(JIT_ARGS) { if (a1->val.s > a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifabo_sf(JIT_ARGS) { if (a1->val.s > a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifabo_fu(JIT_ARGS) { if (a1->val.f > a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifabo_fs(JIT_ARGS) { if (a1->val.f > a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifabo_ff(JIT_ARGS) { if (a1->val.f > a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
...
static void cmd_ifbel_uu(JIT_ARGS) { if (a1->val.u < a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifbel_us(JIT_ARGS) { if (a1->val.u < a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifbel_uf(JIT_ARGS) { if (a1->val.u < a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifbel_su(JIT_ARGS) { if (a1->val.s < a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifbel_ss(JIT_ARGS) { if (a1->val.s < a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifbel_sf(JIT_ARGS) { if (a1->val.s < a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifbel_fu(JIT_ARGS) { if (a1->val.f < a2->val.u) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifbel_fs(JIT_ARGS) { if (a1->val.f < a2->val.s) { NEW_IP(curr_cpu->ip + 1); } }
static void cmd_ifbel_ff(JIT_ARGS) { if (a1->val.f < a2->val.f) { NEW_IP(curr_cpu->ip + 1); } }
...
static void cmd_mod(JIT_ARGS) { a1->val.u%=a2->val.u; }
...
/*! SHL 2 u RW s RO = cmd_shl */
static void cmd_shl(JIT_ARGS) { a1->val.u<<=a2->val.u; }

/*! SHR 2 u RW s RO = cmd_shr */
...
/* GCC IS PATHETIC! MY CAT CAN WRITE BETTER ASM! */
	...
	while (i) {
		...
		a1->val.u=(a1->val.u << 1) | (a1->val.u >> (sizeof(unsigned) - 1));
		i--;
	}
}
...
static void cmd_ror(JIT_ARGS)
{
	...
	while (i) {
		...
		a1->val.u=(a1->val.u << (sizeof(unsigned) - 1)) | (a1->val.u >> 1);
		i--;
	}
}
...
#define PUSH_STACK(a) curr_cpu->mstack_ptr++; \
	if(curr_cpu->mstack_ptr >= curr_cpu->mstack_size) throw_except(curr_cpu, ERR_MSTACK_OVER); \
...
static void cmd_pop_u(JIT_ARGS) { anyval *z;
	POP_STACK(z); a1->val.u=z->val.u; }
static void cmd_pop_s(JIT_ARGS) { anyval *z;
	POP_STACK(z); a1->val.s=z->val.s; }
static void cmd_pop_f(JIT_ARGS) { anyval *z;
...
/*! REALLOC 2 u RO u RO = cmd_realloc_size */
/*! REALLOC 2 u RO s RO = cmd_realloc_perm */
static void cmd_realloc_size(JIT_ARGS) {
	mem_realloc(curr_cpu, a1->val.u, a2->val.u);
}
...
#ifndef NONREENTRANT
	pop_ip_from_stack_mt(curr_cpu, work);
#else
	pop_ip_from_stack_st(work);
#endif
}

/*! HALT 0 = cmd_halt */
static void cmd_halt(JIT_ARGS) {
	printk("=> Task committed suicide (HALT command).\n");
	exit(0);
//	destroy_task(curr_cpu);
}

/*! WAIT 1 u RO = cmd_wait */
/* I hope you meant SLEEPFOR not WAITTILL, lcamtuf? */
static void cmd_wait(JIT_ARGS) {
	curr_cpu->status|=STATUS_CWAIT;
	curr_cpu->wait=a1->val.u;
}
